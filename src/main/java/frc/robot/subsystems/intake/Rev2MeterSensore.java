...
public class Rev2MeterSensore {
    ...
    // sensor list
    private static final List<Rev2MeterSensore> m_sensors = new ArrayList<>();
    ...
    private int m_addr = 0x52;
    private int m_port;

    public Rev2MeterSensore() {
        m_units = Unit.kMillimeters;

        VL53L0XJNI.Init(0, m_addr);
        if(!initialize()) {
            System.out.println("Error Init Intake sensor");
        }
    ...
    public void setAutomaticMode(boolean enabling) {
        ...
        if(enabling) {
            ...
            for(Rev2MeterSensore sensor: m_sensors) {
